...
function AdminEventUpdatePage() {
    ...
    const submit = (values, action) => {
        ...
        dispatch(updateEvent({ eventId: eventId, eventInfo: eventInfo }));
        action.resetForm();
    ...
    useEffect(() => {
        if (event) {
            ...
            setFieldValue("startDate", event.startDate || dayjs().toISOString());
            setFieldValue("endDate", event.endDate || dayjs().toISOString());
            setFieldValue("numberOfTickets", event.numberOfTickets || 0);
            setFieldValue("ticketPrice", event.ticketPrice || 0);
            setFieldValue("isOnSale", event.isOnSale || false);
            setFieldValue("isOnBanner", event.isOnBanner || false);
            setFieldValue("rules", event.rules || "");
        }
    }, [event])
